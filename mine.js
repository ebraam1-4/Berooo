...
function initTypingSubtitle() {
  ...
  subtitle.style.cssText += "opacity:1; border-right:2px solid rgba(212,175,55,0.7); white-space:nowrap; overflow:hidden; display:inline-block;";
  let i = 0;
  setTimeout(() => {
...
function initThemeSwitcher() {
  ...
  body.classList.toggle("light-theme", localStorage.getItem("mm-theme") === "light");

  function updateSwitcher() {
    const isLight = body.classList.contains("light-theme");
    label.textContent = isLight ? "الوضع الفاتح" : "الوضع الداكن";
    icon.textContent  = isLight ? "☀️" : "🌙";
    switcher.style.background = isLight ? "rgba(15,23,30,0.1)" : "rgba(255,255,255,0.08)";
  }
...
function createNavTab(id, href, title) {
  ...
  if (existing) {
    ...
    if (link) { link.href = href; link.textContent = title; }
    return existing;
  }
